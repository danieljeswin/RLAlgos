...
class A2C:
    ...
    def save_network(self):
        torch.save(self.policy_network.state_dict(), self.env_name + "-mya2cgaeactor.pth")
        torch.save(self.value_critic.state_dict(), self.env_name + "-mya2cgaecritic.pth")


    def load_network(self):
        self.policy_network.load_state_dict(torch.load(self.env_name + "-mya2cgaeactor.pth"))
        self.value_critic.load_state_dict(torch.load(self.env_name + "-mya2cgaecritic.pth"))

    def get_discounted_cumulative_sum(self, values, dones, gamma):
        for t in range(self.trajectory_length - 2, -1, -1):
            values[t] += gamma * values[t + 1] * (1 - dones[t])
        return values

    def train(self):
        self.writer = SummaryWriter(comment = '-mya2cgae')
        episode_num = 0
        ...
        while episode_num < self.total_episodes:  
            ...
            gae_values = values.detach().cpu().numpy()    
            state_t = state.copy()
            state_t = torch.Tensor(state_t).to(self.device)
            value = self.value_critic(state_t).item()
            gae_values = np.append(gae_values, value)

            deltas = rewards + self.gamma * gae_values[1:] * (1 - dones) - gae_values[:-1]
            advantages = self.get_discounted_cumulative_sum(deltas, dones, self.gamma * self.gae_lambda)
            advantages = torch.Tensor(advantages).to(self.device)
            actor_loss = -logprob * advantages

            returns = advantages + values
            critic_loss = self.critic_loss_criterion(returns, values) * self.critic_coefficient

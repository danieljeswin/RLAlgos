...
class CombinedDQNAgent:
    def __init__(self, env_name):
        ...
        self.network = dqn_model.DuelingDQNModel(self.env.observation_space.shape, self.env.action_space.n).to(self.device)
        self.target_network = dqn_model.DuelingDQNModel(self.env.observation_space.shape, self.env.action_space.n).to(self.device)
        self.replay_buffer = replay_buffer.NStepPriorityBuffer(self.replay_buffer_size, self.rollout_length, self.gamma, self.alpha)
    ...
    def optimize(self):
        ...
        next_state_actions = self.network(next_states).max(1)[1]
        next_state_values = self.target_network(next_states).gather(1, next_state_actions.unsqueeze(-1)).squeeze(-1)
        next_state_values[terminals] = 0.0
        next_state_values = next_state_values.detach()
        Q_targets = next_state_values * (self.gamma ** self.rollout_length) + rewards
        ...
        while True:
            ...
            if done:
                ...
                if self.best_mean_reward is None or self.best_mean_reward < mean_reward:
                    torch.save(self.network.state_dict(), self.env_name + "-mycombineddqnbest.pth")    
    ...
    def run_test_episode(self):
        ...
        self.network.load_state_dict(torch.load(self.env_name + "-mycombineddqnbest.pth"))
...
if __name__ == "__main__":
    ...
    agent = CombinedDQNAgent(env_name)
    agent.test()